  ...
  return (
    <div className="bg-base-200 rounded-lg shadow-lg p-6">
      <div className="flex items-center mb-4">
        ...
        <h2 className="text-xl font-bold text-primary-content">Quick Calculator</h2>
      </div>
      
      {/* Current Percentage Section */}
      <div className="bg-base-300 p-4 rounded-lg mb-4">
        <h3 className="font-medium mb-3 text-primary-content">Current Percentage</h3>
        <div className="flex gap-3 mb-2">
          <div className="flex-1">
            <input
              type="number"
              placeholder="Attended"
              className={`w-full p-2 rounded input input-bordered ${error ? 'input-error' : ''}`}
              value={attended}
        ...
        <div className="text-right">
          <span className="font-medium text-primary-content">Percentage: {percentage}%</span>
        </div>
      </div>
      
      {/* What If Scenarios Section */}
      <div className="bg-base-300 p-4 rounded-lg mb-4">
        <h3 className="font-medium mb-3 text-primary-content">What If Scenarios</h3>
        <div className="space-y-2">
          <div className="flex justify-between">
            <span className="text-base-content">If I attend next 5 classes</span>
            <span className={scenarios.attend5 >= 0 ? "text-success" : "text-error"}>
              {scenarios.attend5 > 0 ? `+${scenarios.attend5}%` : `${scenarios.attend5}%`}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-base-content">If I miss next 2 classes</span>
            <span className={scenarios.miss2 >= 0 ? "text-success" : "text-error"}>
              {scenarios.miss2 > 0 ? `+${scenarios.miss2}%` : `${scenarios.miss2}%`}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-base-content">If I attend next 10 classes</span>
            <span className={scenarios.attend10 >= 0 ? "text-success" : "text-error"}>
      ...
      <div className="bg-base-300 p-4 rounded-lg">
        <h3 className="font-medium mb-3 text-primary-content">Goal Tracker</h3>
        <div className="space-y-2">
          <div className="flex justify-between">
            <span className="text-base-content">To reach 75%:</span>
            <span className="text-warning">Need {goals.goal75} more</span>
          </div>
          <div className="flex justify-between">
            <span className="text-base-content">To reach 80%:</span>
            <span className="text-warning">Need {goals.goal80} more</span>
          </div>
          <div className="flex justify-between">
            <span className="text-base-content">To reach 85%:</span>
            <span className="text-warning">Need {goals.goal85} more</span>
          </div>
        </div>
      </div>
    </div>
